...
const UserController = {
  ...
  addCard(req, res) {
    ...
    const { ADD_CARD_SUCCESS, INVALID_INPUT, ERROR } = cardService.outputs;

    cardService
      .once(ADD_CARD_SUCCESS, (data) => {
  ...
  getCardsByUserId(req, res) {
    const { cardService } = req.container.cradle;
    const { GET_CARDS_BY_USER_SUCCESS, INVALID_INPUT, ERROR } =
      cardService.outputs;

    cardService
      .once(GET_CARDS_BY_USER_SUCCESS, (data) => {
        res.status(Status.CREATED).json(data);
      })
      .once(INVALID_INPUT, (data) => {
        res.status(Status.BAD_REQUEST).json({
